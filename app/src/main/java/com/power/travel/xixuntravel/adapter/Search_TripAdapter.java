 ...
 *
 */
public class Search_TripAdapter extends BaseAdapter implements OnClickListener {
	...
	public Search_TripAdapter(Context context, List<AllTripModel> list) {
		super();
...
//		.cacheInMemory(true)// �Ƿ񾏴涼�ȴ���
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (!TextUtils.isEmpty(list.get(position).getArr_img())) {//ͼƬ��Ϊ��
			imageLoader.displayImage(list.get(position).getArr_img().split(",")[0],
					holder.face, options,animateFirstListener);
		}
		
		
//		holder.description.setText(list.get(position).get);
