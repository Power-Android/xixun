package com.power.travel.xixuntravel.recordvideo;
...
import com.power.travel.xixuntravel.activity.Video_sendActivity;
...
//        findViewById(R.id.title_left).setOnClickListener(this);
        ...
        rlBottomRootLayoutParams.height = width / 4 * 2;
            ...
            public boolean onTouch(View v, MotionEvent event) {
                if (event.getAction() == MotionEvent.ACTION_DOWN) {
                    ...
                    if (startY - event.getY() > 100) {//上移超过一定距离取消录制，删除文件
                        ...
                        if (movieRecorderView.getTimeCount() > 1) {//录制时间超过三秒，录制完成
                            handler.sendEmptyMessage(RECORD_FINISH);
                        } else {//时间不足取消录制，删除文件
...
//            startActivityForResult(intent, REQ_CODE);
            Intent intent = new Intent(this,Video_sendActivity.class);
			Bundle bundle = new Bundle();
			bundle.putString("videoPath", movieRecorderView.getRecordFile().getAbsolutePath());
