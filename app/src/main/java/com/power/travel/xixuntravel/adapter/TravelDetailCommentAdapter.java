 ...
 *
 */
public class TravelDetailCommentAdapter extends BaseAdapter {
	...
	public TravelDetailCommentAdapter(Context context, List<TravelDetailCommentModel> list){
		super();
...
//		.cacheInMemory(true)// 是否緩存都內存中
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(TextUtils.equals(list.get(position).getComment_id(), "0")){//评论游记
			...
			if(!TextUtils.isEmpty(list.get(position).getComment_upName())){
				SpannableStringBuilder builder = new SpannableStringBuilder("回复"+list.get(position).getComment_upName()+"："+list.get(position).getContent());
				builder.setSpan(redSpan, "回复".length(),"回复".length()+list.get(position).getComment_upName().length(),
