...
import com.power.travel.xixuntravel.adapter.SystemMessageAdapter;
 ...
 *
 */
public class SystemMessageFragment extends Fragment implements OnClickListener,
OnRefreshListener2<ListView>{

	private String data, TAG = "SystemMessageFragment", info, mobile;
	private ProgressDialogUtils pd;
	private PullToRefreshListView mListView;
	List<SystemMessageModel> adapterList = new ArrayList<SystemMessageModel>();
	List<SystemMessageModel> adapterListMore = new ArrayList<SystemMessageModel>();
	...
	private Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			if (msg.what == 1) {// 成功
				...
				if (page == 1) {
					adapter = new SystemMessageAdapter(getActivity(), adapterList);
	...
	public View onCreateView(LayoutInflater inflater,
		...
		View view=inflater.inflate(R.layout.fragment_systemmessage, container, false);
		initView(view);
		if(isConnect()){
			getData(true);
		}else{
	...
	private void getData(boolean ifshow) {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				String url = HttpUrl.system_message;
				String json = StringUtils.setJSON(data);

				LogUtil.e(TAG, "系统消息提交的数据" + json);
				...
				try {
					...
					LogUtil.e(TAG, "系统消息返回的数据" + jsonj.toString());
